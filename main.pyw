...
todo: add way to change key binds
 ...
 some automations happen twice due to running quickly and keys still being pressed
 way to turn automations on/off
"""
from tkinter import *
import time
    ...
    for _ in range(repetitions):
        for raw_action in actions:
            ...
            if action[0] == 'down':
                ...
                old_dir = os.getcwd()
                destination = ''
                for directory in raw_action[5:].split('\\')[:-1]:
                    destination += directory + '\\'
                os.chdir(destination)
                os.system(f'start cmd /c \"{str(raw_action[5:])}\"')
                os.chdir(old_dir)
...
class Window(Tk):
    ...
    def __init__(self, actions):
        ...
        self.not_an_int_error = Label(self, text='Repetitions Must be a Valid Integer', font=('Times New Roman', 18),
                                      fg='red')
...
class Options(Window):
    ...
    def create_home(self):
        ...
        self.remove_widgets()
        self.widgets.append(
            [Button(self, text='Saved Shortcuts', font=('Times New Roman', 18), command=lambda: self.create_saves()), 0, 5, TOP])
        self.widgets.append(
    ...
    def create_settings(self):
        ...
        self.widgets.append([Button(self.widgets[-1][0], text='Save', font=('Times New Roman', 14),
                                    command=lambda: self.save_settings()), 5, 0, LEFT])
        self.widgets.append([Button(self.widgets[-2][0], text='Open Readme', font=('Times New Roman', 14),
                                    command=lambda: self.open_readme()), 5, 0, LEFT])
        self.widgets.append(
            [Button(self.widgets[-3][0], text='Back', font=('Times New Roman', 14), command=lambda: self.create_home()),
             5, 0, RIGHT])
        self.widgets.append([Button(self.widgets[-4][0], text='Open Folder', font=('Times New Roman', 14),
                                    command=lambda: os.startfile(DIRECTORY)), 5, 0, RIGHT])
        self.widgets.append([Frame(self), 0, 10, TOP])
        self.widgets.append([Button(self.widgets[-1][0], text='Delete all shortcuts', font=('Times New Roman', 14),
                                    command=lambda: self.are_you_sure('shortcuts')), 5, 0, LEFT])
        self.widgets.append([Button(self.widgets[-2][0], text='Delete all automations', font=('Times New Roman', 14),
                                    command=lambda: self.are_you_sure('automations')), 5, 0, RIGHT])
        self.place_widgets()
    ...
    # creates are you sure window
    def are_you_sure(self, action):
        self.title('Confirm')
        self.remove_widgets()
        if action == 'shortcuts':
            cmd = self.delete_all_shortcuts
        else:
            cmd = self.delete_all_automations
        self.widgets.append([Label(self, text='Are You Sure?', font=('Times New Roman', 30)), 0, 30, TOP])
        self.widgets.append([Frame(self), 0, 0, TOP])
        self.widgets.append([Button(self.widgets[-1][0], text='Yes', font=('Times New Roman', 30),
                                    command=lambda: cmd()), 30, 0, LEFT])
        self.widgets.append([Button(self.widgets[-2][0], text='No', font=('Times New Roman', 30),
                                    command=lambda: self.create_settings()), 30, 0, RIGHT])
        self.place_widgets()
...
if __name__ == '__main__':
    ...
    README_TXT = 'Keystrokes Shortcuts, How to Use:\n\n    Recording Actions:\n        --> Press ctrl + shift + r to ' \
                 ...
                 'delete all shortcuts, automations and settings)\n        --> If a file in an automation is moved or' \
                 ' renamed, it will fail to run the file\n\n    Upcoming Features:\n        --> Mouse recording (it ' \
                 'is not implemented yet)\n       --> Ability to change keybindings to start recording and open ' \
                 'settings window\n        --> Bug fixes '
